...
- 🛒 **Parts & Workshop Marketplace** — triggered on Critical alerts: AI diagnosis summary (GPT-4o-mini), estimated EUR parts pricing with eBay search links + interactive Google Maps showing nearby workshops in the vehicle's home city
...
When a vehicle's **overall health score drops below 50** (red gauges), a collapsible marketplace panel appears automatically below the Maintenance Recommendations section — closing the loop from alert to action without the user leaving the app.

### 🤖 AI Diagnosis (GPT-4o-mini)

The first thing shown inside the panel is a plain-English diagnosis generated by OpenAI's GPT-4o-mini. It receives the vehicle name, critical component list, and live sensor readings, then returns 2–3 sentences explaining:
- **Why** the components are in critical condition (referencing actual sensor values)
- **What** the fleet manager should do immediately

The API call is cached per unique vehicle + sensor state using `@st.cache_data` — it fires once on first open and is reused on every subsequent re-run, keeping costs minimal.

> Requires `OPENAI_API_KEY` in `.env`. Without it, the diagnosis block is silently skipped and the rest of the panel renders normally.
...
An interactive dark map centred on the vehicle's home city at street level (zoom 13), showing `car_repair` workshops within a 5 km radius. The map opens focused on the city where the vehicle is located.
...
Add keys to your `.env` file (gitignored — never committed):
```
GOOGLE_MAPS_API_KEY=your_google_maps_key_here
OPENAI_API_KEY=your_openai_key_here
```
> Google Maps requires **Places API** and **Geocoding API** enabled in Google Cloud Console.

> Both keys are optional — the app degrades gracefully. Without Google Maps key, the map slot shows an info message. Without OpenAI key, the AI diagnosis block is silently skipped.
...
# 4. (Optional) Enable AI diagnosis + workshop map — add keys to .env
...
- Priority-based maintenance recommendations
- Parts & Workshop Marketplace panel (Critical alerts only) — AI diagnosis (GPT-4o-mini), EUR parts pricing with eBay links + Google Maps workshop search in the vehicle's home city
...
│   ├── ai_diagnosis.py                   # OpenAI GPT-4o-mini plain-English diagnosis
...
├── .env                                  # API keys: GOOGLE_MAPS_API_KEY, OPENAI_API_KEY (gitignored)
├── README.md                             # This file
